...
class AdminMenu {
	...
	private function render_dashboard(): void {
		...
		$lifetime_stats  = $repo->get_summary();
		$last_seven_days = $repo->get_summary(
			array(
				'start_date' => gmdate( 'Y-m-d', strtotime( '-6 days' ) ),
				'end_date'   => gmdate( 'Y-m-d' ),
			)
		);
		$top_rows = $repo->get_top_offers( 5 );
		?>
		<section class="wbcom-suo-screen-hero wbcom-suo-screen-hero-dashboard">
			<div>
				<h2><?php esc_html_e( 'Performance Snapshot', 'wbcom-smart-upsell-order-bump' ); ?></h2>
				<p><?php esc_html_e( 'Dashboard gives a quick business view, while Analytics provides full filtered reporting.', 'wbcom-smart-upsell-order-bump' ); ?></p>
			</div>
			<a class="button button-secondary" href="<?php echo esc_url( admin_url( 'admin.php?page=wbcom-smart-upsell&tab=analytics' ) ); ?>"><?php esc_html_e( 'Open Analytics', 'wbcom-smart-upsell-order-bump' ); ?></a>
		</section>

		<div class="wbcom-suo-section-title"><?php esc_html_e( 'Lifetime Totals', 'wbcom-smart-upsell-order-bump' ); ?></div>
		<div class="wbcom-suo-cards">
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Offer Views', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo esc_html( (string) $lifetime_stats['views'] ); ?></span></div>
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Conversions', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo esc_html( (string) $lifetime_stats['conversions'] ); ?></span></div>
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Conversion Rate', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo esc_html( (string) $lifetime_stats['conversion_rate'] ); ?>%</span></div>
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Revenue', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo wp_kses_post( wc_price( (float) $lifetime_stats['revenue'] ) ); ?></span></div>
		</div>

		<div class="wbcom-suo-section-title"><?php esc_html_e( 'Last 7 Days', 'wbcom-smart-upsell-order-bump' ); ?></div>
		<div class="wbcom-suo-cards wbcom-suo-cards-compact">
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Views (7d)', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo esc_html( (string) $last_seven_days['views'] ); ?></span></div>
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Conversions (7d)', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo esc_html( (string) $last_seven_days['conversions'] ); ?></span></div>
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'CVR (7d)', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo esc_html( (string) $last_seven_days['conversion_rate'] ); ?>%</span></div>
			<div class="wbcom-suo-card"><strong><?php esc_html_e( 'Revenue (7d)', 'wbcom-smart-upsell-order-bump' ); ?></strong><span><?php echo wp_kses_post( wc_price( (float) $last_seven_days['revenue'] ) ); ?></span></div>
		</div>

		<section class="wbcom-suo-panel wbcom-suo-dashboard-table-panel">
			<div class="wbcom-suo-panel-head">
				<h3><?php esc_html_e( 'Top Performing Offers', 'wbcom-smart-upsell-order-bump' ); ?></h3>
				<a class="button button-link" href="<?php echo esc_url( admin_url( 'admin.php?page=wbcom-smart-upsell&tab=analytics' ) ); ?>"><?php esc_html_e( 'View full report', 'wbcom-smart-upsell-order-bump' ); ?></a>
			</div>
			<table class="widefat striped wbcom-suo-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Offer Type', 'wbcom-smart-upsell-order-bump' ); ?></th>
						<th><?php esc_html_e( 'Offer ID', 'wbcom-smart-upsell-order-bump' ); ?></th>
						<th><?php esc_html_e( 'Views', 'wbcom-smart-upsell-order-bump' ); ?></th>
						<th><?php esc_html_e( 'Conversions', 'wbcom-smart-upsell-order-bump' ); ?></th>
						<th><?php esc_html_e( 'Revenue', 'wbcom-smart-upsell-order-bump' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php if ( empty( $top_rows ) ) : ?>
						<tr><td colspan="5"><?php esc_html_e( 'No analytics data yet.', 'wbcom-smart-upsell-order-bump' ); ?></td></tr>
					<?php else : ?>
						<?php foreach ( $top_rows as $row ) : ?>
							<tr>
								<td><?php echo esc_html( (string) $row->offer_type ); ?></td>
	...
	private function render_analytics(): void {
		...
		?>
		<section class="wbcom-suo-screen-hero wbcom-suo-screen-hero-analytics">
			<div>
				<h2><?php esc_html_e( 'Detailed Analytics', 'wbcom-smart-upsell-order-bump' ); ?></h2>
				<p><?php esc_html_e( 'Filter by offer type, product and date range to identify what drives conversion and revenue.', 'wbcom-smart-upsell-order-bump' ); ?></p>
			</div>
		</section>

		<section class="wbcom-suo-panel wbcom-suo-analytics-filter-panel">
		...
		<section class="wbcom-suo-panel wbcom-suo-analytics-table-panel">
			<table class="widefat striped wbcom-suo-table">
	...
	private function render_offer_management( string $tab ): void {
		...
		$is_funnels    = 'funnels' === $tab;
		$active_count  = count(
			array_filter(
				$offers,
				static function ( array $offer ): bool {
					return 'active' === ( $offer['status'] ?? '' );
				}
			)
		);
		$draft_count   = max( count( $offers ) - $active_count, 0 );
		$screen_title  = ucfirst( str_replace( '-', ' ', $tab ) );
		...
		?>
		<div class="wbcom-suo-offers wbcom-suo-offers-<?php echo esc_attr( $tab ); ?>">
			<section class="wbcom-suo-offers-hero">
				<div>
					<div class="wbcom-suo-offers-hero-meta">
						<span class="wbcom-suo-offers-badge"><?php esc_html_e( 'Performance-first', 'wbcom-smart-upsell-order-bump' ); ?></span>
					</div>
					<h2><?php echo esc_html( $screen_title ); ?></h2>
					<p>
						<?php
						if ( 'funnels' === $tab ) {
							esc_html_e( 'Manage cart page funnel offers with conversion-focused, lightweight defaults.', 'wbcom-smart-upsell-order-bump' );
						} elseif ( 'upsells' === $tab ) {
							esc_html_e( 'Configure post-purchase one-click offers and optimize accepted upsells.', 'wbcom-smart-upsell-order-bump' );
						} else {
							esc_html_e( 'Configure checkout order bumps to increase average order value.', 'wbcom-smart-upsell-order-bump' );
						}
						?>
					</p>
					<?php if ( $is_funnels ) : ?>
						<ul class="wbcom-suo-offers-points">
							<li><?php esc_html_e( 'Cart inline, popup and grid layouts', 'wbcom-smart-upsell-order-bump' ); ?></li>
							<li><?php esc_html_e( 'Rule targeting without slowing checkout', 'wbcom-smart-upsell-order-bump' ); ?></li>
							<li><?php esc_html_e( 'Scheduling and skip logic ready', 'wbcom-smart-upsell-order-bump' ); ?></li>
						</ul>
					<?php endif; ?>
				</div>
				<div class="wbcom-suo-offers-metrics">
					<div><span><?php esc_html_e( 'Total', 'wbcom-smart-upsell-order-bump' ); ?></span><strong><?php echo esc_html( (string) count( $offers ) ); ?></strong></div>
					<div><span><?php esc_html_e( 'Active', 'wbcom-smart-upsell-order-bump' ); ?></span><strong><?php echo esc_html( (string) $active_count ); ?></strong></div>
					<div><span><?php esc_html_e( 'Draft', 'wbcom-smart-upsell-order-bump' ); ?></span><strong><?php echo esc_html( (string) $draft_count ); ?></strong></div>
				</div>
			</section>

			<div class="wbcom-suo-offers-layout">
			<section class="wbcom-suo-panel wbcom-suo-offers-list-panel">
				<div class="wbcom-suo-panel-head">
					<h3><?php esc_html_e( 'Existing Offers', 'wbcom-smart-upsell-order-bump' ); ?></h3>
					<div class="wbcom-suo-panel-actions">
						<a class="button button-secondary" href="#wbcom-suo-offer-editor"><?php esc_html_e( 'Create New', 'wbcom-smart-upsell-order-bump' ); ?></a>
						<a class="button button-link" href="<?php echo esc_url( add_query_arg( array( 'page' => 'wbcom-smart-upsell', 'tab' => 'settings' ), admin_url( 'admin.php' ) ) ); ?>"><?php esc_html_e( 'Global Settings', 'wbcom-smart-upsell-order-bump' ); ?></a>
					</div>
				</div>
				<table class="widefat striped wbcom-suo-table">
					...
					<?php else : ?>
						<?php foreach ( $offers as $offer ) : ?>
							<tr>
								...
								<td><span class="wbcom-suo-status wbcom-suo-status-<?php echo esc_attr( $offer['status'] ); ?>"><?php echo esc_html( ucfirst( $offer['status'] ) ); ?></span></td>
								<td><?php echo esc_html( (string) $offer['priority'] ); ?></td>
								<td class="wbcom-suo-table-actions">
									<a class="button-link" href="<?php echo esc_url( add_query_arg( array( 'page' => 'wbcom-smart-upsell', 'tab' => $tab, 'edit_offer' => $offer['id'] ), admin_url( 'admin.php' ) ) ); ?>"><?php esc_html_e( 'Edit', 'wbcom-smart-upsell-order-bump' ); ?></a>
									<a class="button-link-delete" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'wbcom_suo_delete_offer', 'offer_id' => $offer['id'], 'offer_type' => $offer_type ), admin_url( 'admin-post.php' ) ), 'wbcom_suo_delete_offer' ) ); ?>" onclick="return confirm('<?php echo esc_js( __( 'Delete this offer?', 'wbcom-smart-upsell-order-bump' ) ); ?>');"><?php esc_html_e( 'Delete', 'wbcom-smart-upsell-order-bump' ); ?></a>
								</td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
			...
			<section id="wbcom-suo-offer-editor" class="wbcom-suo-panel wbcom-suo-editor">
				...
				<p class="description"><?php esc_html_e( 'Set product, rules, schedule, and advanced logic in one place.', 'wbcom-smart-upsell-order-bump' ); ?></p>
	...
	private function render_settings_form( array $settings ): void {
		?>
		<form method="post" action="options.php" class="wbcom-suo-form wbcom-suo-settings-form">
			<?php settings_fields( 'wbcom_suo_settings_group' ); ?>
			<section class="wbcom-suo-settings-hero">
				<div>
					<h2><?php esc_html_e( 'Settings', 'wbcom-smart-upsell-order-bump' ); ?></h2>
					<p><?php esc_html_e( 'Configure modules, performance and storefront display defaults.', 'wbcom-smart-upsell-order-bump' ); ?></p>
				</div>
				<div class="wbcom-suo-settings-pill"><?php esc_html_e( 'Performance-First Mode', 'wbcom-smart-upsell-order-bump' ); ?></div>
			</section>

			<div class="wbcom-suo-settings-grid">
				<section class="wbcom-suo-settings-card">
					<h3><?php esc_html_e( 'Modules', 'wbcom-smart-upsell-order-bump' ); ?></h3>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[enable_order_bumps]" value="1" <?php checked( ! empty( $settings['enable_order_bumps'] ) ); ?>> <span><?php esc_html_e( 'Enable checkout order bumps', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[enable_post_purchase_upsell]" value="1" <?php checked( ! empty( $settings['enable_post_purchase_upsell'] ) ); ?>> <span><?php esc_html_e( 'Enable post-purchase upsells', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[enable_cart_bumps]" value="1" <?php checked( ! empty( $settings['enable_cart_bumps'] ) ); ?>> <span><?php esc_html_e( 'Enable cart bumps', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[enable_analytics]" value="1" <?php checked( ! empty( $settings['enable_analytics'] ) ); ?>> <span><?php esc_html_e( 'Enable analytics tracking', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
				</section>

				<section class="wbcom-suo-settings-card">
					<h3><?php esc_html_e( 'Display & Performance', 'wbcom-smart-upsell-order-bump' ); ?></h3>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[lazy_load_scripts]" value="1" <?php checked( ! empty( $settings['lazy_load_scripts'] ) ); ?>> <span><?php esc_html_e( 'Lazy load scripts', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[disable_reports]" value="1" <?php checked( ! empty( $settings['disable_reports'] ) ); ?>> <span><?php esc_html_e( 'Disable reports', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[animation]" value="1" <?php checked( ! empty( $settings['animation'] ) ); ?>> <span><?php esc_html_e( 'Enable animation effects', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-toggle-row"><input type="checkbox" name="wbcom_suo_settings[cache_mode]" value="1" <?php checked( ! empty( $settings['cache_mode'] ) ); ?>> <span><?php esc_html_e( 'Cache compatibility mode', 'wbcom-smart-upsell-order-bump' ); ?></span></label>
					<label class="wbcom-suo-field-row" for="wbcom-suo-template-style"><?php esc_html_e( 'Default Template', 'wbcom-smart-upsell-order-bump' ); ?></label>
				...
				<section class="wbcom-suo-settings-card wbcom-suo-settings-preview-card">
					<h3><?php esc_html_e( 'Live Preview', 'wbcom-smart-upsell-order-bump' ); ?></h3>
					<p class="description"><?php esc_html_e( 'This style is applied as your storefront default template.', 'wbcom-smart-upsell-order-bump' ); ?></p>
					<div class="wbcom-suo-template-preview-wrap">
						<div id="wbcom-suo-template-preview" class="wbcom-suo-offer wbcom-suo-template-<?php echo esc_attr( $settings['template_style'] ?? 'minimal' ); ?>">
							...
							<p><?php esc_html_e( 'Add this one-time offer and increase average order value instantly.', 'wbcom-smart-upsell-order-bump' ); ?></p>
			...
			<div class="wbcom-suo-settings-actions">
				<?php submit_button( __( 'Save Settings', 'wbcom-smart-upsell-order-bump' ), 'primary', 'submit', false ); ?>
			</div>
		</form>
		<?php
	}
