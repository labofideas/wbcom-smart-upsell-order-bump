...
class AnalyticsRepository {
	...
	public function get_summary( array $filters = array() ): array {
		...
		$views       = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table} WHERE action_type = 'view'{$sql_where}" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,PluginCheck.Security.DirectDB.UnescapedDBParameter
		$conversions = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table} WHERE action_type = 'accept'{$sql_where}" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,PluginCheck.Security.DirectDB.UnescapedDBParameter
		$revenue     = (float) $wpdb->get_var( "SELECT SUM(revenue) FROM {$table} WHERE action_type = 'accept'{$sql_where}" ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,PluginCheck.Security.DirectDB.UnescapedDBParameter
	...
	public function get_top_offers( int $limit = 10, array $filters = array() ): array {
		...
		$sql = "SELECT offer_type, offer_id,
		...
		$rows = $wpdb->get_results( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,PluginCheck.Security.DirectDB.UnescapedDBParameter
	...
	private function build_where_clause( array $filters ): array {
		...
		if ( ! empty( $filters['offer_type'] ) ) {
			$parts[] = "offer_type = '" . esc_sql( sanitize_key( (string) $filters['offer_type'] ) ) . "'";
		}

		if ( ! empty( $filters['offer_id'] ) ) {
			$parts[] = 'offer_id = ' . absint( $filters['offer_id'] );
		}
		...
		if ( ! empty( $filters['product_id'] ) ) {
			...
			if ( empty( $offer_ids ) ) {
				...
				$parts[] = 'offer_id IN (' . implode( ',', array_map( 'absint', $offer_ids ) ) . ')';
			}
		}

		if ( ! empty( $filters['start_date'] ) ) {
			$parts[] = "DATE(created_at) >= '" . esc_sql( sanitize_text_field( (string) $filters['start_date'] ) ) . "'";
		}

		if ( ! empty( $filters['end_date'] ) ) {
			$parts[] = "DATE(created_at) <= '" . esc_sql( sanitize_text_field( (string) $filters['end_date'] ) ) . "'";
		}
	...
	private function get_offer_ids_for_product( int $product_id ): array {
		...
		$table = $wpdb->prefix . 'wbcom_suo_offers';
